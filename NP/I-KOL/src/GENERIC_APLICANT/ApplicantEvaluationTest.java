...
class TwoCondition implements Evaluator{
    Evaluator evaluator1;
    Evaluator evaluator2;


    public TwoCondition(Evaluator evaluator1, Evaluator evaluator2) {
...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        Evaluator applicantWithNoCriminalRecord = applicant -> !applicant.hasCriminalRecord();
        Evaluator withMoreExpericence = applicant -> applicant.getEmploymentYears() >= 10;
        Evaluator withMoreCreditScore = applicant -> applicant.getCreditScore() >= 500;


        Evaluator noCriminalAndMoreExperience = new TwoCondition(applicantWithNoCriminalRecord, withMoreExpericence);
        Evaluator moreExpAndCredit = new TwoCondition(withMoreExpericence, withMoreCreditScore);
        Evaluator noCriminalAndScore = new TwoCondition(applicantWithNoCriminalRecord, withMoreCreditScore);


        switch (type) {
            case NO_CRIMINAL_RECORD:
                return applicantWithNoCriminalRecord;
            case MORE_EXPERIENCE:
                return withMoreExpericence;
            case MORE_CREDIT_SCORE:
                return withMoreCreditScore;
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return noCriminalAndMoreExperience;
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                return moreExpAndCredit;
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                return noCriminalAndScore;
            default:
