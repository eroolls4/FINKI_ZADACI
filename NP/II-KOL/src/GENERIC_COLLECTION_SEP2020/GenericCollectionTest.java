package GENERIC_COLLECTION_SEP2020;
...
class GenericCollection<T extends IHasTimestamp & Comparable<T>>{
   ...
   Map<String,List<T>> elementsByCategory;


    public GenericCollection() {
        elementsByCategory=new HashMap<>();
    }

    void addGenericItem (String category, T element){
        elementsByCategory.putIfAbsent(category,new ArrayList<>());
...
//     return    elementsByCategory.entrySet()
//                                 .stream()
//                                 .filter(entry -> categories.contains(entry.getKey()))
//                                 .flatMap(category-> elementsByCategory.get(category).stream())
//                                 .collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
